...
@Component({
  selector: 'app-company-growth-chart',
  standalone: false,
  templateUrl: './company-growth-chart.component.html',
  styleUrl: './company-growth-chart.component.css'
})
export class CompanyGrowthChartComponent implements OnInit {
 ...
 jobPostingsPerMonth: number[] = [];
  ...
  loadData() {
    ...
    this.adminDashboardService.getCompanysPerMonth().subscribe({
      next: (data: number[]) => {
      ...
      error: (error: any) => {
        console.error('Error fetching company growth per month', error);
        this.errorMessage = 'Unable to load company growth data. Please try again later.';
        this.isLoading = false;
      }
    });
  }
